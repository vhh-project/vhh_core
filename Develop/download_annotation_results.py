...
output_filename = "annotation_result"
...
for result in annotation_results:
    ...
    for i, row in enumerate(result["data"]):
        result["data"][i] = {"shot_id": shot_id, "start": row['inPoint'], "end": row["outPoint"], "stc": row["shotType"], "cmc": row["cameraMovement"]}
        shot_id += 1
...
for result in annotation_results:
    ...
    for row in result["data"]:
        annotation_results_single_file.append({"vid_name": id, "shot_id": row["shot_id"], "start": row['start'], "end": row["end"], "stc": row["stc"], "cmc": row["cmc"]})
...
if args.store_separate_files:
    for result in annotation_results:
        id = result["id"]
        data = result["data"]
        file_name_without_extension = os.path.join(args.path, output_filename) + "_id" + str(id)
        if args.store_as_csv:
            file_path = Utils.make_filepath_unique(file_name_without_extension, '.csv')
