"""
Download the relations for a given VID.
Use the command with the "-h" paramter to find out more about how to use it.

Example use:
    python download_relations.py -i 8470  -p /data/share/fjogl/relations_test/
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description="Download relations")
...
# Download
for id in ids:
    data = RestAPI.getRelations(id)

    if data == []:
        print(f"Film {id} has no relations. No files will be generated.")
        continue

    path = os.path.join(args.path, f"relations_{id}.json")
